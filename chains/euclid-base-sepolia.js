...
const config = {
  ...
  chainId: 84532,
  name: 'Base Sepolia',
  contract: '0xc3b9297130E49E0E514884cca150435a1324865b',
  explorer: 'https://sepolia.basescan.org/tx/',
  tokens: ['usdc', 'usdt', 'mon', 'stt'],
  chainUid: 'base'
};
...
const processSwap = async (privateKey, swapType, numTransactions, minEthAmount, maxEthAmount, minDelay, maxDelay, requireConfirmation, logger) => {
  ...
  const escrowPromises = ['euclid', 'mon', 'usdc', 'usdt', 'stt'].map(async (token) => {
  ...
  const possibleTargetTokens = ['euclid', 'mon', 'usdc', 'usdt', 'stt'];
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        if (!routes || routes.length === 0) {
          logger.error(`[✓] [${config.name}] ❌ No routes found for ETH -> ${currentToken}. Trying next token...`);
          break;
        }
...
// Helper: checkSwapStatus (Base Sepolia swap pending log)
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (response.data?.response?.is_completed) {
        logger.success(`[Base Sepolia] Swap completed: ${txHash}`);
        return true;
      }
      if (SHOW_SWAP_PENDING_LOG.base) {
        logger.loading(`[Base Sepolia] Swap pending (Attempt ${attempt}/10): ${txHash}`);
      }
    } catch (error) {
      logger.warn(`[Base Sepolia] Status check failed: ${error.message}. Verifying on-chain...`);
      try {
        const receipt = await provider.getTransactionReceipt(txHash);
        if (receipt && receipt.status === 1) {
          logger.success(`[Base Sepolia] Swap confirmed on-chain: ${txHash}`);
          return true;
        }
      } catch (onChainError) {
        logger.debug(`[Base Sepolia] On-chain verification failed: ${onChainError.message}`);
      }
    }
    await randomDelay(delay, delay + 5000);
  }
  logger.error(`[Base Sepolia] Swap did not complete after ${maxAttempts} attempts: ${txHash}`);
  return false;
};
