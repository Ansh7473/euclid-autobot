...
const config = {
  ...
  chainId: 59141,
  name: 'Linea Sepolia',
  contract: '0xd2711D3ce6ccBf44ea25F285CE115C0905761360',
...
// Helper: checkSwapStatus (Linea Sepolia swap pending log)
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (response.data?.response?.is_completed) {
        logger.success(`[Linea Sepolia] Swap completed: ${txHash}`);
        return {
    success: successCount > 0,
    successCount,
    failCount,
    totalSwaps: numTransactions
      ...
      logger.warn(`[Linea Sepolia] Status check failed: ${error.message}. Verifying on-chain...`);
      try {
        const receipt = await provider.getTransactionReceipt(txHash);
        if (receipt && receipt.status === 1) {
          logger.success(`[Linea Sepolia] Swap confirmed on-chain: ${txHash}`);
          return {
    success: successCount > 0,
    successCount,
    failCount,
    totalSwaps: numTransactions
        ...
        logger.debug(`[Linea Sepolia] On-chain verification failed: ${onChainError.message}`);
      }
    }
    await randomDelay(delay, delay + 5000);
  }
  logger.error(`[Linea Sepolia] Swap did not complete after ${maxAttempts} attempts: ${txHash}`);
  return {
    success: false,
    successCount: 0,
    failCount: numTransactions,
