...
const config = {
  ...
  explorer: 'https://www.megaexplorer.xyz/tx/',
  tokens: ['euclid', 'usdc', 'usdt', 'eth'],
  chainUid: 'megaeth'
};
...
const processSwap = async (privateKey, swapType, numTransactions, minEthAmount, maxEthAmount, minDelay, maxDelay, requireConfirmation, logger) => {
  ...
  for (const token of ['eth', 'euclid', 'usdc', 'usdt']) {
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    while (attempt < maxAttempts) {
      try {
        logger.info(`[✓] [${config.name}] 🎯 Selected token: ${currentToken} (chain: ${currentChainUid})`);
        logger.info(`[✓] [${config.name}] 🔄 Transaction ${i + 1}/${numTransactions} (ETH → ${currentToken.toUpperCase()}, ${ethAmount} ETH):`);
        logger.info(`[✓] [${config.name}] 🗺️  Requesting route: ETH → ${currentToken.toUpperCase()}`);
        ...
        if (!routes || routes.length === 0) {
          logger.warn(`[✓] [${config.name}] ⚠️  No routes found for ETH → ${currentToken.toUpperCase()}. Trying next token...`);
          break;
        }
        ...
        if (receipt.status === 1) {
          ...
          logger.success(`[✓] [${config.name}] 🎯 Route: ETH → ${currentToken.toUpperCase()}`);
          ...
          logger.success(`[✓] [${config.name}] ✅ Transaction tracked successfully`);
          ...
          logger.error(`[✓] [${config.name}] 🎯 Failed route: ETH → ${currentToken.toUpperCase()}`);
          break;
        }
      } catch (error) {
        failCount++;
        ...
        if (error.response?.status === 429) {
          logger.warn(`[✓] [${config.name}] ⚠️  Rate limit encountered. Retrying after delay...`);
      ...
      if (attempt < maxAttempts) {
        logger.loading(`[✓] [${config.name}] 🔄 Retrying swap for ${currentToken} (Attempt ${attempt + 1}/${maxAttempts})...`);
...
const validateTokenRoutes = async (token, chainUids, amountInWei, logger) => {
  try {
    const routesPayload = {
      ...
      intermediate_tokens: ['euclid', 'usdc', 'usdt'],
    };
    const response = await axios.post(
...
// Helper: checkSwapStatus (MegaETH swap pending log)
const checkSwapStatus = async (txHash, chainUid, walletAddress, provider, logger, meta, maxAttempts = 10, delay = 30000) => {
  ...
  logger.error(`[✓] [MegaETH] Swap did not complete after ${maxAttempts} attempts: ${txHash}`);
  return false;
};
