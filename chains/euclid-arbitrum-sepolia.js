...
const processSwap = async (privateKey, swapType, numTransactions, minEthAmount, maxEthAmount, minDelay, maxDelay, requireConfirmation, logger) => {
  ...
  for (const token of ['euclid', 'eth', 'usdc', 'usdt']) {
  ...
  const possibleTargetTokens = ['euclid', 'eth', 'usdc', 'usdt'];
...
const validateTokenRoutes = async (token, chainUids, amountInWei, logger) => {
  try {
    const routesPayload = {
      ...
      intermediate_tokens: ['euclid', 'eth', 'usdc', 'usdt'],
    };
    const response = await axios.post(
...
const checkSwapStatus = async (txHash, chainUid, walletAddress, provider, logger, meta, maxAttempts = 10, delay = 8000) => {
