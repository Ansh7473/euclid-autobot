...
const config = {
  ...
  explorer: 'https://testnet.mintscan.io/osmosis-testnet/tx/',
  // Only native OSMO token for same-chain swaps
  nativeTokens: ['osmo'],
  // EVM tokens for cross-chain swaps
  evmTokens: ['euclid', 'usdc', 'usdt', 'mon', 'stt'],
  chainUid: 'osmosis',
  denom: {
    osmo: 'uosmo'
  },
...
// Get random token pair for cross-chain swap (OSMO -> EVM tokens)
function getRandomTokenPair() {
  // Always start with OSMO (native token)
...
// Main swap process
const processOsmosisSwap = async (
  privateKey,
  swapType,
  numSwaps,
  minAmount,
...
export default processOsmosisSwap;
