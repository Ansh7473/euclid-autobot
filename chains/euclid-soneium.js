...
const config = {
  ...
  explorer: 'https://soneium-minato.blockscout.com/tx/',
  tokens: ['euclid', 'usdc', 'usdt', 'stt'],
  chainUid: 'soneium'
};
...
const processSwap = async (privateKey, swapType, numTransactions, minEthAmount, maxEthAmount, minDelay, maxDelay, requireConfirmation, logger) => {
  ...
  for (const token of ['euclid', 'stt', 'usdc', 'usdt']) {
...
// Helper: checkSwapStatus (Soneium Minato Testnet swap pending log)
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (response.data?.response?.is_completed) {
        logger.success(`[Soneium Minato Testnet] Swap completed: ${txHash}`);
        return {
    success: successCount > 0,
    successCount,
    failCount,
    totalSwaps: numTransactions
      ...
      if (SHOW_SWAP_PENDING_LOG.soneium) {
        logger.loading(`[Soneium Minato Testnet] Swap pending (Attempt ${attempt}/10): ${txHash}`);
      }
    } catch (error) {
      logger.warn(`[Soneium Minato Testnet] Status check failed: ${error.message}. Verifying on-chain...`);
      try {
        const receipt = await provider.getTransactionReceipt(txHash);
        if (receipt && receipt.status === 1) {
          logger.success(`[Soneium Minato Testnet] Swap confirmed on-chain: ${txHash}`);
          return {
    success: successCount > 0,
    successCount,
    failCount,
    totalSwaps: numTransactions
        ...
        logger.debug(`[Soneium Minato Testnet] On-chain verification failed: ${onChainError.message}`);
      }
    }
    await randomDelay(delay, delay + 5000);
  }
  logger.error(`[Soneium Minato Testnet] Swap did not complete after ${maxAttempts} attempts: ${txHash}`);
  return {
    success: false,
    successCount: 0,
    failCount: numTransactions,
